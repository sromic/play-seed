...
import repositories.{ OAuth2InfoDao, SlickDao }
...
class OAuth2InfoDaoImpl @Inject()(
  ...
  protected def oAuth2InfoQuery(loginInfo: LoginInfo) =
    for {
  ...
  protected def addAction(loginInfo: LoginInfo, authInfo: OAuth2Info) =
    loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      slickOAuth2Infos += DBOAuth2Info(None,
  ...
  def find(loginInfo: LoginInfo): Future[Option[OAuth2Info]] = {
    ...
    result.map { dbOAuth2InfoOption =>
      dbOAuth2InfoOption.map { dbOAuth2Info =>
        OAuth2Info(dbOAuth2Info.accessToken,
                   dbOAuth2Info.tokenType,
                   dbOAuth2Info.expiresIn,
                   dbOAuth2Info.refreshToken)
      }
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    ...
    val action = query.result.head.flatMap {
      case (dbLoginInfo, Some(dbOAuth2Info)) =>
        updateAction(loginInfo, authInfo)
      case (dbLoginInfo, None) =>
        addAction(loginInfo, authInfo)
    }.transactionally
